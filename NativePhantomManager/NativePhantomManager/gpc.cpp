...
void GpcPoly::reset_it(it_node **it)
{
  it_node *itn;
...
void GpcPoly::reset_lmt(lmt_node **lmt)
{
  lmt_node *lmtn;
...
void GpcPoly::insert_bound(edge_node **b, edge_node *e)
{
...
edge_node** GpcPoly::bound_list(lmt_node **lmt, double y)
{
...
void GpcPoly::add_to_sbtree(int *entries, sb_tree **sbtree, double y)
{
  if (!*sbtree)
  {
...
void GpcPoly::build_sbt(int *entries, double *sbt, sb_tree *sbtree)
{
  if (sbtree->less)
...
void GpcPoly::free_sbtree(sb_tree **sbtree)
{
  if (*sbtree)
  {
...
int GpcPoly::count_optimal_vertices(gpc_vertex_list c)
{
  int result= 0, i;
...
edge_node* GpcPoly::build_lmt(lmt_node **lmt, sb_tree **sbtree, int *sbt_entries, gpc_polygon *p, int type, gpc_op op)
{
...
void GpcPoly::add_edge_to_aet(edge_node **aet, edge_node *edge, edge_node *prev)
{
  if (!*aet)
  {
...
void GpcPoly::add_intersection(it_node **it, edge_node *edge0, edge_node *edge1, double x, double y)
{
...
void GpcPoly::add_st_edge(st_node **st, it_node **it, edge_node *edge, double dy)
{
...
void GpcPoly::build_intersection_table(it_node **it, edge_node *aet, double dy)
{
  st_node   *st, *stp;
  edge_node *edge;
...
int GpcPoly::count_contours(polygon_node *polygon)
{
  int          nc, nv;
  vertex_node *v, *nextv;
...
void GpcPoly::add_left(polygon_node *p, double x, double y)
{
  vertex_node *nv;
...
void GpcPoly::merge_left(polygon_node *p, polygon_node *q, polygon_node *list)
{
  polygon_node *target;
...
void GpcPoly::add_right(polygon_node *p, double x, double y)
{
  vertex_node *nv;
...
void GpcPoly::merge_right(polygon_node *p, polygon_node *q, polygon_node *list)
{
  polygon_node *target;
...
void GpcPoly::add_local_min(polygon_node **p, edge_node *edge, double x, double y)
{
...
int GpcPoly::count_tristrips(polygon_node *tn)
{
  int total;
...
void GpcPoly::add_vertex(vertex_node **t, double x, double y)
{
  if (!(*t))
  {
...
void GpcPoly::new_tristrip(polygon_node **tn, edge_node *edge, double x, double y)
{
  if (!(*tn))
  {
...
bbox* GpcPoly::create_contour_bboxes(gpc_polygon *p)
{
  bbox *box;
  int   c, v;
...
void GpcPoly::minimax_test(gpc_polygon *subj, gpc_polygon *clip, gpc_op op)
{
  bbox *s_bbox, *c_bbox;
...
void GpcPoly::gpc_free_polygon(gpc_polygon *p)
{
  int c;
...
void GpcPoly::gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
  int c, v;
...
void GpcPoly::gpc_write_polygon(FILE *fp, int write_hole_flags, gpc_polygon *p)
{
  int c, v;
...
void GpcPoly::gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
...
void GpcPoly::gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                      gpc_polygon *result)
{
  sb_tree       *sbtree= NULL;
    ...
    for (poly= out_poly; poly; poly= npoly)
    {
      ...
      if (poly->active)
      {
        ...
        for (vtx= poly->proxy->v[LEFT]; vtx; vtx= nv)
        {
		  ...
		  result->contour[c].vertex[v].z = clip->contour[0].vertex[0].z;
...
void GpcPoly::gpc_free_tristrip(gpc_tristrip *t)
{
  int s;
...
void GpcPoly::gpc_polygon_to_tristrip(gpc_polygon *s, gpc_tristrip *t)
{
  gpc_polygon c;
...
void GpcPoly::gpc_tristrip_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                       gpc_tristrip *result)
{
  sb_tree       *sbtree= NULL;
  it_node       *it= NULL, *intersect;
